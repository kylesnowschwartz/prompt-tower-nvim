...
function M.select_current_file()
  ...
  if success then
    vim.notify(string.format('Added "%s" to Prompt Tower selection', vim.fn.fnamemodify(current_file, ':t')), vim.log.levels.INFO)
  else
    vim.notify(string.format('Could not select "%s" - file not found in workspace', vim.fn.fnamemodify(current_file, ':t')), vim.log.levels.WARN)
  end
end
...
function M.generate_context()
  ...
  vim.notify(string.format('Generated context with %d files and copied to clipboard', #selected_files), vim.log.levels.INFO)
end
...
function M.toggle_selection(filepath)
  ...
  elseif not new_state and was_selected then
    ...
    vim.notify(string.format('Could not toggle selection for "%s"', vim.fn.fnamemodify(target_file, ':t')), vim.log.levels.WARN)
  end
end
